//#define EDITABLE_HIERARCHY_FOLDER_TRANSFORM // Allow moving, rotating or scaling hierarchy folders using the editor tools
//#define DEBUG_INIT
...
namespace Sisus.HierarchyFolders
{
	public sealed class HierarchyFolderManager
	{
		...
		private readonly List<HierarchyFolder> hierarchyFolders = new();
		private readonly HashSet<HierarchyFolder> destroying = new();
		...
		internal void OnValidate(HierarchyFolder hierarchyFolder)
		{
			...
			var gameObject = hierarchyFolder.gameObject;
			if(HierarchyFolder.gameObjects.Add(gameObject))
			{
				Register(hierarchyFolder);
			}

			HierarchyFolder.infos[gameObject.GetInstanceID()] = new(hierarchyFolder);
		}
		...
		internal void OnAwake(HierarchyFolder hierarchyFolder)
		{
			...
			var gameObject = hierarchyFolder.gameObject;
			bool unregistered = HierarchyFolder.gameObjects.Add(gameObject);
			HierarchyFolder.infos[gameObject.GetInstanceID()] = new(hierarchyFolder);
			...
			var transformHideFlags = GetHierarchyFolderTransformHideFlags(transform);
		...
		private void Deregister(HierarchyFolder hierarchyFolder)
		{
			...
			var gameObject = hierarchyFolder.gameObject;
			HierarchyFolder.gameObjects.Remove(gameObject);
			HierarchyFolder.infos.Remove(gameObject.GetInstanceID());
		...
		private void OnHierarchyFolderDetectedOnAPrefabAndNotAllowed(HierarchyFolder hierarchyFolder, bool isInstance)
		{
			...
			if(!hierarchyFolder || !destroying.Add(hierarchyFolder))
			{
				return;
			}
