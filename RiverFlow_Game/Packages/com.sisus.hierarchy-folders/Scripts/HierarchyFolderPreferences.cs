...
namespace Sisus.HierarchyFolders
{
...
#endif
	{
		...
		[FormerlySerializedAs("allowInactiveHierarchyFolders")]
		[Tooltip("Settings for active toggle in the Inspector for Hierarchy Folders:" +
		         "\n\nAffects Children: the active toggle affects the children of the hierarchy folder instead of the hierarchy folder itself. Hierarchy Folders can't ever be set as inactive." +
		         "\n\nAffects Hierarchy Folder: the active toggle affects the hierarchy folder itself. Note that when inactive hierarchy folders are stripped from scene hierarchies, their child game objects can become active in the hierarchy." +
		         "\n\nHidden: the active toggle is hidden in the Inspector. Hierarchy Folders can't ever be set as inactive.")]
		public ActiveToggle activeToggle = ActiveToggle.AffectsChildren;
		...
		{
			AffectsChildren = 0,
			AffectsHierarchyFolder = 1,
			Hidden = 2
		}

#if UNITY_EDITOR

		public static bool FlattenHierarchy => EditorApplication.isPlayingOrWillChangePlaymode && Get().playModeBehaviour == StrippingType.FlattenHierarchy;
