...
#endif
#if UNITY_2022_2_OR_NEWER
        handle = pass.AllocLayerHandle(camera, layerScaleVec2, pass.resultGraphicsFormat, pass.destinationFilterMode, vrUsage, dimension, slices);
#else
        handle = pass.AllocLayerHandle(camera, layerScaleVec2, pass.resultGraphicsFormat, pass.destinationFilterMode, dimension, slices);
#endif
...
#if UNITY_2022_2_OR_NEWER
    public RTHandle AllocLayerHandle(Camera camera, Vector2 scaleFactor, GraphicsFormat colorFormat, FilterMode filterMode, VRTextureUsage vrUsage, TextureDimension dimension, int slices)
    {
        var handleSystem = GetHandleSystem(camera);
        return handleSystem.Alloc(scaleFactor, wrapMode: TextureWrapMode.Clamp, colorFormat: colorFormat, memoryless: RenderTextureMemoryless.Depth, filterMode: filterMode, useMipMap: false, msaaSamples: MSAASamples.None, vrUsage: vrUsage, dimension: dimension, slices: slices);
    }
#else
    public RTHandle AllocLayerHandle(Camera camera, Vector2 scaleFactor, GraphicsFormat colorFormat, FilterMode filterMode, TextureDimension dimension, int slices)
    {
        var handleSystem = GetHandleSystem(camera);
        return handleSystem.Alloc(scaleFactor, wrapMode: TextureWrapMode.Clamp, colorFormat: colorFormat, memoryless: RenderTextureMemoryless.Depth, filterMode: filterMode, useMipMap: false, msaaSamples: MSAASamples.None, dimension: dimension, slices: slices);
    }
#endif
    ...
    private void Setup(Camera camera, RenderTextureDescriptor descriptor, float renderScale)
    {
        ...
        if (FlexibleBlurFeature.ImageBasedBlurDict.TryGetValue(key, out var blurredImageAreas) && blurredImageAreas is { Count: > 0 })
        {
            ...
            while (currentRTHandleList.Count < numLayerTextures)
            {
                ...
                {
#if UNITY_2022_2_OR_NEWER
                    currentRTHandleList.Add(handleSystem.Alloc(layerTextureScaleFactor, wrapMode: TextureWrapMode.Clamp, colorFormat: resultGraphicsFormat, memoryless: RenderTextureMemoryless.Depth, filterMode: destinationFilterMode, useMipMap: false, msaaSamples: MSAASamples.None, vrUsage: vrUsage, dimension: dimension, slices: slices));
#else
                    currentRTHandleList.Add(handleSystem.Alloc(layerTextureScaleFactor, wrapMode: TextureWrapMode.Clamp, colorFormat: resultGraphicsFormat, memoryless: RenderTextureMemoryless.Depth, filterMode: destinationFilterMode, useMipMap: false, msaaSamples: MSAASamples.None, dimension: dimension, slices: slices));
#endif
                }
            }
